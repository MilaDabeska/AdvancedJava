...
class Scheduler<T> {
    ...
    public boolean remove(Timestamp<T> t) {
        //го брише соодветниот елемент од распоредувачот доколку постои
        // и враќа true, во спротивно враќа false
    ...
    public Timestamp<T> next() {
        //тој објект чие што време е најблиску до тековното (сега) и сѐ уште НЕ е поминато
        ...
        for (int i = 0; i < timestamps.size(); i++) {
            ...
            if (now.isBefore(current) && current.isBefore(next)) {
                next = current;
                element = timestamps.get(i).getElement();
            }
        }
        return new Timestamp<>(next, element);
    }

    public Timestamp<T> last() {
        // го враќа објектот кој има време најблиску до тековното (сега) и веќе E поминат
        LocalDateTime now = LocalDateTime.now();
        ...
        for (int i = 0; i < timestamps.size(); i++) {
            ...
            if (current.isBefore(now) && current.isAfter(last)) {
                last = timestamps.get(i).getTime();
                element = timestamps.get(i).getElement();
            }
        }
        return new Timestamp<>(last, element);
    }

    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end) {
        //враќа листа на настани чии времиња се наоѓаат помеѓу begin и end
        // (не вклучувајќи ги begin и end).
        List<Timestamp<T>> result = new ArrayList<>();
        LocalDateTime current;
        for (int i = 0; i < timestamps.size(); i++) {
            current = timestamps.get(i).getTime();
