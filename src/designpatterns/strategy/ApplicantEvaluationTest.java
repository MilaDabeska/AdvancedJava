...
class EvaluatorBuilder {
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        ...
        if (type.equals(Evaluator.TYPE.NO_CRIMINAL_RECORD))
            return evaluator = applicant -> !applicant.hasCriminalRecord();
        if (type.equals(Evaluator.TYPE.MORE_EXPERIENCE))
            return evaluator = applicant -> applicant.getEmploymentYears() > 10;
        if (type.equals(Evaluator.TYPE.MORE_CREDIT_SCORE))
            return evaluator = applicant -> applicant.getCreditScore() >= 500;
        if (type.equals(Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE))
            return evaluator = applicant -> !applicant.hasCriminalRecord() && applicant.getEmploymentYears() > 10;
        if (type.equals(Evaluator.TYPE.MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE))
            return evaluator = applicant -> applicant.getEmploymentYears() > 10 && applicant.getCreditScore() >= 500;
        if (type.equals(Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE))
            return evaluator = applicant -> !applicant.hasCriminalRecord() && applicant.getCreditScore() >= 500;
